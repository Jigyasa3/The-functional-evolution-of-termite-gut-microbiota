...
print(paste0("The max. contamination of MAGs is ",min(maginfo6$Contamination)))
...
reductive<-read.csv("basicstats/mag_reductiveacetogenesis.csv")
...
reductive2<-reductive%>%select("Bin.IDs","fhs","folD","ftcD","metF","fdhF1","pta","ack","acsABCDE_2")
#add all gene counts-
reductive2$totalsum<-reductive2$fhs+reductive2$folD+reductive2$ftcD+reductive2$metF+reductive2$fdhF1+reductive2$pta+reductive2$ack+reductive2$acsABCDE_2
...
reductive_mags<-merge(reductive2,maginfo,by="Bin.IDs")

#get MAGs with more than 5 reductive genes-
reductive_mags2<-reductive_mags%>%filter(totalsum>=5)
write.csv(reductive_mags2,file="reductiveacetogenesis_summedbygenes_march2021.csv")

#supplementary table S12-
reductive_sup<-merge(reductive,maginfo,by="Bin.IDs")
write.csv(reductive_sup,file="reductiveacetogenesis_supptable12.csv")
#-------------------------------------------------------------------------------------------------
##STATS-

#how many mags have more than 5 genes present?
print(paste0("There are ",reductive_mags2%>%filter(totalsum>=5)%>%select(Bin.IDs)%>%nrow(), " MAGs with more than equal to 5 gene-families of Reductive acetogenesis"))
...
reductive_mags2%>%filter(Phyla=="Desulfobacterota"& Bin.IDs %in% sulfate2$BinID)%>%nrow()
reductive_mags2%>%filter(Phyla=="Desulfobacterota")%>%nrow()

print(paste0("There are", reductive_mags2%>%filter(Phyla=="Desulfobacterota")%>%nrow(), " Desulfobacterota MAGs, out of which", reductive_mags2%>%filter(Phyla=="Desulfobacterota"& Bin.IDs %in% sulfate2$BinID)%>%nrow(), " has sulfate reducing aprAB genes"))
